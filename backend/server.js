...
// In-Memory Database (Stateful across requests)
let db = {
  user: {
    id: 'user-001',
...
app.post('/api/auth/login', (req, res) => {
  ...
  return res.json({
    ...
    token: 'jwt_token_qlpl_black_edition',
    user: db.user
  });
});
...
app.post('/api/computers', (req, res) => {
  ...
  const newId = (id || name).toUpperCase().trim();
  ...
  const newComputer = {
    ...
    cpu: cpu || 'Intel Core i5-12400',
    ram: ram || '16GB DDR4',
    gpu: gpu || 'NVIDIA GTX 1660',
    status: status || 'available',
    currentUser: null,
    notes: notes || 'Thiết bị mới đưa vào vận hành'
  };
...
app.listen(PORT, () => {
  console.log(`QLPL Backend API running at http://localhost:${PORT}`);
});
